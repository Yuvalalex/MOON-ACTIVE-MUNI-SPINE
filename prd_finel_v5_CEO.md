...
- לוח הזמנים נשען על שימוש ביכולות קיימות, היעדר מיגרציות מסוכנות ו-blast radius נמוך.
<details>
...
- הגדרת Non-Goals נועדה להגן על לוחות הזמנים, הכלכלה והאבטחה של ה-MVP.
...
- רגע החזרה לרשת ממותג כנקודת שיא חיובית שמחזקת סיפוק ומונטיזציה.
...
- כאשר המכסה נגמרת, החוויה נשארת מכבדת ומרגיעה במקום לחסום באגרסיביות.
...
- נוסחי הממשק הרשמיים נשמרו בהרחבה כדי לשמור על הקריאה הניהולית ממוקדת.
...
- השרת בודק אילו הישגים הושגו לפני סיום האירוע ומזכה בהתאם לאחר החיבור.
...
- התגמול מותאם לרמת הכפר ולמכפיל, כדי לשמור על קוהרנטיות כלכלית בין שחקנים.
<details>
<summary>📖 להרחבה - פרטים טכניים מלאים</summary>
...
- עיקרון הוגנות: אופליין נשאר רשת ביטחון, לא ערוץ עדיף על אונליין.
...
- מנגנון Jitter מפזר reconnect המוני לאורך שניות בודדות כדי להימנע מ-spike מסוכן.
...
- יש גם יכולת השבתה גלובלית מיידית וגם מנגנוני בלימה אוטומטיים מקומיים.
<details>
<summary>📖 להרחבה - פרטים טכניים מלאים</summary>
...
- הטבלה המלאה נשמרה בהרחבה; הכאן-ועכשיו הוא ההבחנה בין אימפקט מיידי לאופק תשתיתי רחב.
...
- החזר ההשקעה מחושב רק על בסיס תרומה נטו אינקרמנטלית, לא על הכנסה ברוטו.
...
- מילון האירועים המלא נשמר להרחבה, בעוד עקרונות המדידה נשארים גלויים.
...
- מוגדרים כאן תרחישי קיצון שמטרתם להוכיח שאין אובדן כלכלי, קריסות או ניצול לרעה.
...
- ההשקה מתקדמת מ-5% ל-50% ואז ל-100%, רק לאחר עמידה ביעדי guardrail.
...
- שירות הלקוחות מקבל שקיפות ובקרות, אך ללא אפשרות לזיכוי חופשי בלתי מבוקר.
...
- הטאב הייעודי נועד לסייע באבחון, תיעוד ופיצוי מבוקר בלבד.
...
- מרוכזות כאן ארבע שאלות הנהלה קריטיות: ניצול לרעה, תשלומים, עומס שרת ורגולציה.
- התשובות המלאות נשמרו בהרחבה, אך המסר המרכזי: אופליין נשלט, מדוד ומוגבל.
- שאלה גלויה: "האם זה לא יעודד שחקנים להתנתק בכוונה כדי לשחק מול בוטים קלים?"
- שאלה גלויה: "מה קורה אם שחקן רוכש חבילת IAP של $99 בטיסה ומבטל את כרטיס האשראי בנחיתה?"
- שאלה גלויה: "האם עבודת ה-Replay לא תיצור צוואר בקבוק כבד על שרתי ה-Backend?"
- שאלה גלויה: "האם מתן ספינים לפני סליקה סופית עומד בהנחיות חנויות האפליקציות?"
...
- המלצת העבודה: להתייחס למהלך כניסוי מוצרי-הנדסי מדורג עם שערי החלטה ברורים.
...
- הפרק מבהיר מה נכנס ל-MVP ומהם התנאים המחייבים כדי להכריז על מוכנות.

### 21.1 גבולות ה-MVP

- ה-MVP מתמקד ברציפות משחק בטוחה; Offline IAP ותוספות מתקדמות נשארים מחוץ לטווח.
...
- תנאי ההשלמה המלאים נשמרו בהרחבה וכוללים אבטחה, כלכלה, תפעול, פרטיות וניסוי.
...
- מוגדר כאן סט האיומים שדורש owner, detection ו-mitigation לפני rollout.
...
- הבקרות מבוססות על attestation, binding, nonce, אימות שרת וזיהוי rollback.
...
- הקו המנחה: איסוף מינימלי, retention מוגבל, הצפנה ועמידה ברגולציה רלוונטית.
...
- מוצגים רק ה-endpoints ההכרחיים: lease, replay, status ו-disable.
...
- העקרונות: idempotency, ordering חתום, atomicity ו-compensation במקרה חריג.
...
- לכל סיכון מרכזי הוגדר owner, mitigation ושער החלטה לפני הרחבת ההשקה.
...
- הנספח מרכז סכמות וקוד ייחוס כדי לשמור את גוף המסמך העסקי תמציתי.
...
- שדות הטוקן והמבנה המלא נשמרו בהרחבה לצורכי הנדסה ואבטחה.
...
- קוד הייחוס נשמר בהרחבה; המסר הניהולי הוא אימות מהיר, דטרמיניסטי וללא נעילת DB.
